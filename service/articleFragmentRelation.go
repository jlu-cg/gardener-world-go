...
const (
	...
	queryArticleFragmentRelationDocumentDetailsSQL = "select a.id, a.position, b.id, b.title, b.content, c.detail_introduction_id from article_fragment_relation a inner join fragment b on a.fragment_id=b.id left join fragment_introduction_relation c on a.fragment_id=c.fragment_id where a.article_id=$1 order by a.position asc"
...
func queryArticleFragmentRelationDocumentDetails(articleID int) []ArticleFragmentRelationDocumentDetail {
	...
	for rows.Next() {
		...
		rows.Scan(&temp.ID, &temp.Position, &temp.FragmentID, &temp.Title, &temp.Content, &temp.DetailIntroductionId)
